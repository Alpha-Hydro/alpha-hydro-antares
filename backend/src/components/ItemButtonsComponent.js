...
export default class PanelButtonsComponent extends React.Component{
	...
	render() {
		...
		const Buttons = [
			...
			{icon: "trash", enable: true, action: "delete", role: "admin", title:"Удалить"},
			{icon: "eye-close", enable: this.props.dataItem.active, action: "disabled", role: "admin", title:"Скрыть"},
			{icon: "eye-open", enable: !this.props.dataItem.active, action: "enabled", role: "admin", title:"Показать"}
		].map((button, i) =>
		...
		return (
			<div className="btn-group btn-group-sm mt1">
				{Buttons}
				<ModalComponent
