...
class Model_Static_PdfBook {
     ...
     * @param bool $init Initialize page? draw HR
     * 
     * @return Model_Static_PdfPage First page
     */
...
//		$page -> drawTextBlock($curRow, -20, $page -> getHeight() - $offset - $productHeight);
	...
	if($this->format == 'A4' && $this->print){	
		...
		if(count($products) <= $count_exceptions || count($products) == 0){
			$this->logger->log('Not products in '.$category->id.' categories', Zend_Log::INFO);//если товаров в категории нет пропускаем ее счет
		}else{
