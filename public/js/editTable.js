...
(function() {
  (function($) {
    $.fn.editTable = function(options) {
      var $table, Draw, Edit, Mode, defaults, settings;
      $table = this;
      defaults = {
        ...
        columns: {
          edittabledata: function() {
            ...
            return $td.each(function() {
              ...
              input = "<input class='form-control input-sm hidden " + settings.inputClass + "' type='text' name='" + settings.columns.editable[i][1] + "' value='" + ($(this).text()) + "' disabled/>";
          ...
          toolbar: function() {
            ...
            return $tr.each(function() {
              return $(this).append("<td class='text-right'>" + toolbar + "</td>");
            });
          }
        }
      };
      Mode = {
        view: function(td) {
          ...
          $input.blur().addClass('hidden').prop('disabled', true);
          $(td).find('span').show();
          $(td).attr('data-mode', 'view');
          if (settings.editButton) {
            $tr.find('button.edittable-save-button').addClass('hidden');
            return $tr.find('button.edittable-edit-button').removeClass('active').blur();
          }
        },
        edit: function(td) {
          var $input, $tr;
          ...
          $(td).find('span').hide();
          $input = $(td).find('input.' + settings.inputClass);
          $input.prop('disabled', false).removeClass('hidden').focus();
          $(td).attr('data-mode', 'edit');
          if (settings.editButton) {
            $tr.find('button.edittable-edit-button').addClass('active');
            return $tr.find('button.edittable-save-button').removeClass('hidden');
          }
        }
      };
      Edit = {
        reset: function(td) {
          return $(td).each(function() {
            var $input, text;
            $input = $(this).find('input.' + settings.inputClass);
            text = $(this).find('span').text();
            $input.val(text);
            return Mode.view(this);
          });
        },
        submit: function(td) {
          var serialize;
      ...
      settings.onDraw();
      if (settings.editButton) {
        $table.on('click', 'button.edittable-edit-button', function(event) {
          var $button, activated;
          if (event.handled !== true) {
            ...
            if (!activated) {
              $($button.parents('tr').find('td[data-mode]').get().reverse()).each(function() {
                return Mode.edit(this);
              });
            }
            return event.handled = true;
          }
        });
        $table.on('click', 'button.edittable-save-button', function(event) {
          if (event.handled !== true) {
            event.preventDefault();
            Edit.submit($(this).parents('tr').find('td[data-mode = "edit"]'));
            return event.handled = true;
          }
        });
      }
